...
The nested daemon's `/var/lib/docker` is **always independent of the home and workspace**. Keep a separate `dev-agent-${SANDBOX_NAME}-docker` volume to retain the nested images, build cache and inner containers across container recreation. If you intentionally omit that mount, the nested daemon's data lives only in the outer container's writable layer.

## Experimental project-home image: rootless Podman

After building the trial image, start with a host checkout and one home volume **plus the nested Docker data volume**:
...
To use an independently persistent, filesystem-disconnected project, replace the bind mount with `--mount "type=volume,src=dev-agent-${SANDBOX_NAME}-workspace,dst=/workspace"`. That adds a separate **workspace** volume. You may instead omit the workspace mount to import a repository into the container's writable `/workspace`; copy it out before removing or upgrading that container. Omit the home volume too only for a fully disposable experiment; the `-docker` volume remains optional according to whether nested Docker storage must persist. Do not add the old separate agent/forge volume mounts on top of a project home.
...
For either engine, `start -ai` resumes the **same image**, not a newer one. To apply an updated versioned home seed, stop the container, preserve its workspace if it is on the writable layer, remove **only the container object**, and recreate using the new locally built image with the same home and Docker-data volume names. See [HOME-VOLUME.md](HOME-VOLUME.md#paths-and-lifecycle). Do not delete existing volumes as part of an image upgrade.
